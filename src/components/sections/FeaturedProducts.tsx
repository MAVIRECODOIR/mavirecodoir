...
function ProductCard({ product }: { product: Product }) {
  ...
  const imageUrl = product.thumbnail || 'https://via.placeholder.com/1600x1797';

  return (
    <Link href={`/pr/${product.handle}`} className="group block">
      <div className="relative overflow-hidden bg-gray-100 aspect-[1600/1797] rounded-sm">
        <img 
          ...
          loading="lazy" 
        />
        <div className="absolute top-3 left-3 bg-[#262626] text-white text-[11px] font-medium tracking-wider uppercase px-3 py-1 rounded-sm">Featured</div>
      ...
      <div className="mt-3 md:mt-4">
        <h3 className="text-sm md:text-base font-medium tracking-[-0.02em] leading-tight">{product.title}</h3>
        <p className="text-sm md:text-base font-medium tracking-[-0.02em] text-[#4f4f4f] mt-1">{price ? formatPrice(price.amount, price.currency_code) : 'Price unavailable'}</p>
      </div>
    </Link>
  );
}
...
function ScrollCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/pr/${product.handle}`} className="group block flex-shrink-0 w-[45vw] sm:w-[30vw] md:w-[22vw] snap-start">
      <div className="relative overflow-hidden bg-gray-100 aspect-[1600/1797] rounded-sm">
        <img 
          src={imageUrl} 
          alt={product.title} 
      ...
      <div className="mt-3">
        <h3 className="text-sm font-medium tracking-[-0.02em] leading-tight">{product.title}</h3>
        <p className="text-sm font-medium tracking-[-0.02em] text-[#4f4f4f] mt-1">{price ? formatPrice(price.amount, price.currency_code) : 'Price unavailable'}</p>
      </div>
    </Link>
  );
}
...
export default function FeaturedProducts() {
  ...
  const hasExtras = products.length > GRID_THRESHOLD;
      ...
      <section className="py-20 md:py-28">
        <div className="luxury-container">
          <div className="text-center mb-12 md:mb-16">
            <p className="text-sm tracking-widest uppercase opacity-60 mb-3">(Featured)</p>
