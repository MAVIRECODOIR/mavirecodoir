...
import FilterSortDrawer from "@/app/[countryCode]/[locale]/men/new/FilterSortDrawer";
...
const GENDER_TAGS = ["men", "women", "unisex", "accessories"] as const;
const CATEGORY_TAGS = ["outerwear", "jackets", "shirts", "t-shirts", "denim", "knitwear", "trousers", "dresses", "tops", "skirts", "bags", "scarves", "hats", "belts"] as const;
...
function extractOptionFilters(products: any[]) {
...
function extractTagFilters(products: any[]) {
  ...
  products.forEach((product: any) => {
    (product.tags || []).forEach((tag: any) => {
      ...
      if (GENDER_TAGS.includes(val)) genders.add(val);
      if (CATEGORY_TAGS.includes(val)) categories.add(val);
    });
  });

  const groups: { name: string; options: { label: string; value: string }[] }[] = [];
  if (genders.size > 0) {
    groups.push({
      name: "Department",
      options: Array.from(genders).map((v) => ({
  ...
  if (categories.size > 0) {
    groups.push({
      name: "Category",
      options: Array.from(categories).map((v) => ({
...
export default async function NewArrivalsPage({ searchParams }: { searchParams?: Promise<{ sort?: string }> }) {
  ...
  const filters = [...extractOptionFilters(products), ...extractTagFilters(products)];
        ...
        <div className="MuiBox-root">
          <div className="MuiBox-root" style={{ paddingTop: "80px" }}>
            <div className="MuiBox-root" style={{ maxWidth: "538px", margin: "0 auto", textAlign: "center", padding: "0 16px" }}>
              ...
              >
                Discover the latest additions across every department
              </p>
            </div>
          </div>
