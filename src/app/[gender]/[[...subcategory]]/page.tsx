  ...
  products.forEach((product) => {
    if (product.options) {
      product.options.forEach((option: any) => {
        if (!filterMap[option.name]) filterMap[option.name] = new Set();
        option.values.forEach((value: string) => filterMap[option.name].add(value));
      });
    }
  });
  return Object.entries(filterMap).map(([name, values]) => ({
    name,
    options: Array.from(values).map((value) => ({ label: value, value: value.toLowerCase().replace(/\s+/g, "-") })),
  }));
}
...
export default async function CollectionPage({ params, searchParams }: { params: Promise<{ gender: string; subcategory?: string[] }>; searchParams?: Promise<{ sort?: string }> }) {
  ...
  // Validate gender against known genders
  const VALID_GENDERS = ["men", "women", "unisex", "accessories"]
  if (!VALID_GENDERS.includes(gender)) return notFound()

  // Derive handle from slug pattern
  const handle = subcategoryParts.length
    ? `${gender}-${subcategoryParts.join("-")}`
    : gender;
